...
if [ $EXPIRED -gt 0 ]
then
	#echo "CRITICAL - expired passwords count = $EXPIRED ($EXPIRED_NAMES) ; expiring soon = $EXPIRE_SOON ($EXPIRE_SOON_NAMES)" | mailx -r "root" -s "Expired passwords on server `hostname`" unix-admins@example.co >/dev/null 2>&1
